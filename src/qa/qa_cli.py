...
from src.retrieval.access_control import DEFAULT_CLEARANCE, SENSITIVITY_LEVELS, max_sensitivity_of_chunks
...
def _print_retrieval_details(result: dict) -> None:
    ...
    if not chunks:
        ...
        for idx, chunk in enumerate(chunks, start=1):
            ...
            print(f"    [{idx}] score={score:.4f}  sensitivity={sensitivity}  source={source}")
            print(f"        \"{preview}\"")
    print("  --------------------------\n")


def _run_single_question(args, dsn: str) -> None:
    result = answer_question(args.index, args.question, top_k=args.top_k, pgvector_dsn=dsn, clearance=args.clearance)
